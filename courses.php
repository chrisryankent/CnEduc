...
foreach ($courses as $course) {
    ...
    // Extract department from course description or name
    $course_lower = strtolower($course['name'] . ' ' . $course['description']);
    if (strpos($course_lower, 'computer') !== false || strpos($course_lower, 'programming') !== false) {
        $departments['Computer Science'] = ($departments['Computer Science'] ?? 0) + 1;
    } elseif (strpos($course_lower, 'business') !== false || strpos($course_lower, 'economics') !== false) {
        $departments['Business'] = ($departments['Business'] ?? 0) + 1;
    } elseif (strpos($course_lower, 'engineering') !== false) {
        $departments['Engineering'] = ($departments['Engineering'] ?? 0) + 1;
    } elseif (strpos($course_lower, 'medical') !== false || strpos($course_lower, 'medicine') !== false) {
        $departments['Medicine'] = ($departments['Medicine'] ?? 0) + 1;
    } elseif (strpos($course_lower, 'law') !== false || strpos($course_lower, 'legal') !== false) {
        $departments['Law'] = ($departments['Law'] ?? 0) + 1;
    } else {
        $departments['Other'] = ($departments['Other'] ?? 0) + 1;
    }
}
?>
...
<body>
        ...
        <div class="grid">
            ...
            <main class="main">
                ...
                <div class="card fade-in">
                    <h2><i class="fas fa-search"></i> Find Your Course</h2>
                    <div class="search-box">
                        <input type="text" placeholder="Search courses by name, department, or keyword..." id="courseSearch">
                        <button class="btn btn-primary btn-sm">
                            <i class="fas fa-search"></i>
                        </button>
                    </div>
                    <div style="margin-top: 20px; display: flex; gap: 12px; flex-wrap: wrap;">
                        <span style="padding: 8px 16px; background: var(--gray-100); border-radius: var(--radius); font-size: 14px; cursor: pointer;" onclick="filterCourses('all')">All</span>
                        <?php foreach (array_keys($departments) as $dept): ?>
                            <span style="padding: 8px 16px; background: var(--gray-100); border-radius: var(--radius); font-size: 14px; cursor: pointer;" onclick="filterCourses('<?php echo strtolower($dept); ?>')"><?php echo htmlspecialchars($dept); ?></span>
                        <?php endforeach; ?>
                    </div>
                </div>
                ...
                <div class="card fade-in delay-1">
                    ...
                    <div class="grid-2">
                        ...
                        ?>
                            <a href="units.php?course_id=<?php echo $course['id']; ?>" class="simple-card fade-in">
                                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                                    <div style="width: 50px; height: 50px; border-radius: 50%; background: rgba(15, 98, 254, 0.1); display: flex; align-items: center; justify-content: center;">
                                        <i class="fas fa-graduation-cap" style="color: var(--accent); font-size: 20px;"></i>
                                    </div>
                                    <div>
                                        <h3 style="margin: 0; font-size: 18px; color: var(--accent);"><?php echo htmlspecialchars($course['name']); ?></h3>
                                        <p style="margin: 4px 0 0 0; font-size: 12px; color: var(--gray-500);"><?php echo $unit_count; ?> Units</p>
                                    </div>
                                </div>
                                <p style="color: var(--gray-600); font-size: 14px; line-height: 1.5;"><?php echo htmlspecialchars($course['description']); ?></p>
                            </a>
                        <?php endforeach; ?>
                    </div>
                </div>
                ...
                <!-- ALL COURSES -->
                <div class="card fade-in delay-2" id="courses">
                    ...
                    <?php if (!empty($courses)): ?>
                        <div class="grid-2" style="margin-top: 24px;">
                            ...
                            ?>
                                <a href="units.php?course_id=<?php echo $course['id']; ?>" class="course-card fade-in">
            ...
            <aside class="sidebar">
                ...
                <div class="card fade-in delay-1">
                    <h3><i class="fas fa-filter"></i> Departments</h3>
                    <ul class="clean-list">
                        <?php foreach ($departments as $dept => $count): ?>
                            <li>
                                <a href="#" onclick="filterCourses('<?php echo strtolower($dept); ?>'); return false;">
                                    <i class="fas fa-folder"></i>
                                    <?php echo htmlspecialchars($dept); ?>
                                    <span style="margin-left: auto; background: var(--gray-200); padding: 2px 8px; border-radius: 10px; font-size: 12px;"><?php echo $count; ?></span>
                                </a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                ...
                <div class="card fade-in delay-1">
                    ...
                    <ul class="clean-list">
                        ...
                        <li><a href="curriculum.php"><i class="fas fa-book"></i> School Curriculum</a></li>
                ...
                <div class="card fade-in delay-2">
                    ...
                    <p style="font-size: 14px; color: var(--gray-700); line-height: 1.6;">
                        University courses require consistent study habits. Create a weekly study schedule, join study groups, and don't hesitate to ask questions during office hours.
                    </p>
                </div>
            </aside>
        </div>
    ...
    <script>
        ...
        document.getElementById('courseSearch').addEventListener('input', function(e) {
            ...
            const courseCards = document.querySelectorAll('.course-card, .simple-card');
        ...
        function filterCourses(department) {
            ...
            courseCards.forEach(card => {
                ...
                if (department === 'all') {
                    card.style.display = 'block';
                } else if (courseName.includes(department) || courseDescription.includes(department)) {
                    card.style.display = 'block';
                } else {
                    card.style.display = 'none';
                }
