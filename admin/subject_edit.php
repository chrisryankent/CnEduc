...
if (isset($_POST['name'])) {
    if (!verify_csrf_token($_POST['csrf_token'])) {
        ...
        if (!$error) {
            ...
            $sql = "UPDATE subjects SET class_id = $class_id, name = '$name', description = '$description', position = $position WHERE id = $subject_id";
            if ($mysqli->query($sql)) {
...
<div class="card">
  ...
  <form method="post">
      ...
      <label>Class<br>
        <select name="class_id" required>
          ...
          <?php foreach ($classes as $c): ?>
            <option value="<?php echo $c['id']; ?>" <?php echo $c['id'] === $subject['class_id'] ? 'selected' : ''; ?>>
    ...
    <div>
      <label>Subject Name<br><input type="text" name="name" value="<?php echo htmlspecialchars($subject['name']); ?>" required style="width:100%;"></label>
    </div>
    <div>
      <label>Description<br><input type="text" name="description" value="<?php echo htmlspecialchars($subject['description']); ?>" style="width:100%;"></label>
    </div>
    <div>
      <label>Position<br><input type="number" name="position" value="<?php echo $subject['position']; ?>"></label>
    </div>
    <div>
